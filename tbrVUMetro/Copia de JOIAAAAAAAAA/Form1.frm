...
Private Sub cboDev_Click()
    cboRecLine.Clear
    'select device cierra el mixer de entrada de grabacion que este elegido
    'y carga el que queremos, si no hay error da true
    If Not clsRecorder.SelectDevice(cboDev.ListIndex) Then
        MsgBox "Couldn't select device!", vbExclamation
        Exit Sub
    End If
    'muestra los canales de grabacion de la placa elegida
    ShowLines
End Sub
...
Private Sub cboRecLine_Click()
    ...
    '*******************************************************
    'PUEDO ELEGIR YO EL STEREO MIXER O EL QUE QUIERA !!!!
    '*******************************************************
...
Private Sub cboSamplerate_KeyPress(KeyAscii As Integer)
...
Private Sub chkFX_Click(Index As Integer)
...
Private Sub SetFormEnabled(bln As Boolean)
    frmOut.Enabled = bln
    frmInp.Enabled = bln
End Sub
...
Private Sub Form_Load()
    ...
    'el clsSmooth devuelve el promedio de una lista de valores
    
    Set clsVUSmoothL = New clsSmooth
    Set clsVUSmoothR = New clsSmooth
    
    'el clsrecorder es un mounstro que usa SubClassing que no entiendo
    'abre ademas un archivo para gravar el archivo
    Set clsRecorder = New WaveInRecorder
    
    'otro mosunstro para grabar en distintos formatos
    Set clsEncoder = New EncoderWAV
    
    'funciones para dibujar la onda de diferentes formas
    ...
    'el vector que contiene todos los valores de la onda
    ReDim intSamples(FFT_SAMPLES - 1) As Integer

    'para saber si es compatible con el formato en que se desea grabar
    blnLoaded = True

    'actualiza en pantalla el encode que se va a usar
    UpdateWAVFmtDisp
    
    'lista de todas las placas de sonido
    ShowDevices
...
Private Sub ShowDevices()
    ...
    cboDev.Clear
    'agrega los nombres de todas las placas de sonido
...
Private Sub ShowLines()
    ...
    cboRecLine.Clear
    'devuelve los mixers de grabacion ' mic, stereo mixer, cd audio, etc
    For i = 0 To clsRecorder.MixerLineCount - 1
        cboRecLine.AddItem clsRecorder.MixerLineName(i)
    Next
    'ya sabe cual esta elegido!!!!
...
Private Sub sldVol_Scroll()
    '*******************************************************
    'PUEDO ELEGIR YO EL STEREO MIXER O EL QUE QUIERA VOLUMEN INCLUIDO !!!!
    '*******************************************************
