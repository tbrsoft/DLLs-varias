...
' 09.02.2006: ID3v2 Position + Length returned in ID3Tag
'             (rm)
' 14.02.2006: formated the code
'             (rm)


'***** The ID3 tag for data input / output
'      with this module *****
Public Type ID3Tag
    Artist          As String
    Album           As String
    ...
    ' The following items are only used with
    ' ID3 v2 tags.
    EncodedBy       As String
    LinkTo          As String
    Copyright       As String
...
'The v 2.4 footer also uses this structure,
' since it's a copy of the header.
...
' The frames stored in memory, used when
' updating a currently existing v2 tag.
Private mMemFrames()    As v2MemFrame
Private mMemFraCnt      As Long
...
Private bBuiltGenre     As Boolean
...
Public Function ReadID3v1( _
    ByVal FileName As String, _
    ByRef outTag As ID3Tag _
) As Boolean
    ...
    ' The ID3v1 tag is located 128 bytes
    ' from the end of the file, so look
    ' at this position.
...
Public Function WriteID3v1( _
    ByVal FileName As String, _
    ByRef inTag As ID3Tag, _
    Optional ByVal bIgnoreReadOnly As Boolean = True _
) As Boolean
    ...
    ' The ID3v1 tag is located 128 bytes from the
    ' end of the file, so look at this position.
    If fsize >= 128 Then
        SetFilePointer fh, fsize - 128, 0, FILE_BEGIN
...
Public Function DeleteID3v1( _
    ...
    ' The ID3v1 tag is located 128 bytes from
    ' the end of the file, so look at this position.
    SetFilePointer fh, fsize - 128, 0, FILE_BEGIN
    ReadFile fh, rdBuf(0), 3, 0, ByVal 0
...
Public Function ReadID3v2( _
    ByVal FileName As String, _
    ByRef outTag As ID3Tag _
) As Boolean

    Dim fh          As Long, fp     As Long
    Dim TagFooter   As v2TagHeader
    Dim rdBuf(3)    As Byte
    ...
    If Not ReadID3v2 Then
        ...
        ' Version 2.4 allows the ID3 tag to be
        ' appended to the audio data. So, look if we
        ' can find such a tag, identified by a footer.
        ' According to id3.org, the ID3v2 footer
        ' should be added before other tagging
        ' systems, therefore prepended to an
        ' possibly existing ID3v1 tag.
        ...
        If Data2String(VarPtr(TagFooter.Identifier(0)), 3, ENC_ISO) = "3DI" Then
            ' Got a footer. Calculate the header
            ' position from the size.
...
' Returns the file pointer for the next tag,
' or -1 if there is no tag appended.
Private Function ReadV2Tag( _
    ByVal fh As Long, _
    ByVal fp As Long, _
    ByRef RdTag As ID3Tag, _
    ByVal bToMem As Boolean _
) As Long

    Dim arrp            As Long
    Dim TagSizes        As v2TagSizes
    Dim TagHeader       As v2TagHeader
    Dim TagHeaderFlags  As v2TagHeaderFlags
    Dim ExtHeader23     As v2_3ExtHeader
    Dim ExtHeader24     As v2_4ExtHeader
    Dim FraHeader22     As v2_2FrameHeader
    Dim FraHeader234    As v2_34FrameHeader
    Dim FraDataSize     As Long
    Dim FraUnsynchDecr  As Long
    Dim fraID           As String
    Dim FraText         As String
    Dim FraOffset       As Long
    Dim bSkipFra        As Boolean
    Dim TagData()       As Byte
    ...
    ' The size stored in the header excludes itself,
    ' and excludes the footer (if present).
    ...
    If TagHeader.Version(0) = 4 Then
        If TagHeader.Flags And &H10& Then
            ...
            ' (which is the same size than the header)
            ' to the read size.
            ' Padding must not exist when a footer
            ' is present.
            TagSizes.ReadSize = TagSizes.ReadSize + TagSizes.FooterSize
        End If
    End If

    RdTag.size = TagSizes.ReadSize

    ' Read in the rest of the tag.
    ReDim TagData(TagSizes.ReadSize - 1)
    ReadFile fh, TagData(0), TagSizes.ReadSize, 0, ByVal 0

    ' Perform de-unsynchoronisation of the whole
    ' read tag, if nescessary.
    ' In version 2.2 and 2.3, the unsynch is done
    ' for all data after the header if the flag is set.
    ' In version 2.4, the unsynch is applied on a
    ' per frame base.
    ...
    Do
        If TagHeader.Version(0) = 2 Then
            ...
            ' Check if there's enough space to read
            ' another tag header. We don't have to
            ' take care of a footer here because
            ' v 2.2 doesn't support footers.
            ...
            If bToMem Then
                ...
                ' Only supported in v 2.2, not
                ' converted.
                If Not fraID = "CRM" And FraDataSize > 0 Then

                    ' Store this frame in the memory buffer,
                    ' we only need the data to update
        ...
        ElseIf TagHeader.Version(0) >= 3 Then
            ...
            If fraID = String$(4, Chr(0)) Then
                ' Got the padding. Frame parsing is
                ' finished now.
                Exit Do
            End If
            ...
            ' v 2.4: Synchsafe integer.
            ' However, there are sloppy programs
            ' which code version 2.4 with
            ' non-synchsafe integers.
            ' Do a test therefore, by checking
            ' if the next frame is a valid one.
            ...
            If TagHeader.Version(0) = 3 Then
                ...
                If FraHeader234.Flags(1) And &H20& Then
                    ' This frame belongs to a group.
                    ' I don't care about that, but
                    ' we have to consider about the
                    ' 1 byte added to the header
            ...
            Else
                ...
                If FraHeader234.Flags(1) And &H1& Then
                    ...
                    ' Is ignored, but add 4 bytes
                    ' offset, because it
                    ' appends a synchsafe integer to
                    ' the header.
            ...
            If Not bSkipFra Then
                ...
                ' Skipped frames also are NOT added
                ' to the memory, because the new
                ' frames are written with a frame
                ' header that doesn't include the
                ' flags for compression or encryption.
                If bToMem And Not fraID = "SEEK" Then

                    ' Store this frame in the memory
                    ' buffer, we only need the data
                    ' to update a currently existing
                    ' tag. The v 2.4 seek frame isn't
                    ' copied to memory, because the
                    ' new written file is going to
                    ' have different offsets anyway.
                    ' However, read out the frame to
                    ' ensure processing of the appended tag.
        ...
        ElseIf fraID = "SEEK" Then
            If IsNumeric(FraText) Then
                ' FraText actually has to be numeric,
                ' because its value is generated from
                ' Data2Long.
...
Private Function ReadFrame( _
    ...
    ElseIf FrameID = "SEEK" Then
        ...
        ' Got a seek frame (v 2.4 only).
        ' It points to the next tag in the file.
        ' According to id3.org, it seems that this
        ' value isn't synchsafe... ??
        ReadFrame = Data2Long(pData, False)

    End If
End Function

Private Function IsValidFrameName( _
    ByVal pData As Long _
) As Boolean
    ...
    For i = 0 To 3
        CopyMemory curData, ByVal pData + i, 1
         ' Accept $00 $00 $00 $00 as a valid
         ' frame name, because this would be
...
' Unsynch is simply done by replacing &HFF &H00
' with &HFF. Returns the number of &H00 bytes
' kicked out.
Private Function DeUnsynch( _
    ByVal pData As Long, _
    ByVal Length As Long _
) As Long
...
' Converts a frame in memory from v 2.2 to
' v 2.3 in order to safe it in a new version.
Private Sub ConvertV23( _
    ByVal memIdx As Long _
)
    ...
    With mMemFrames(memIdx)
        ...
        If .ID = "PIC" Then
            ...
            ' Convert the picture type desc from
            ' v 2.2 to 2.3 (and 2.4)
        ...
        ElseIf .ID = "LNK" Then
            ...
            ' Convert the link frame from v 2.2 to
            ' 2.3 (and 2.4)
        ...
        ElseIf .ID = "TDOR" Then
            ...
            ' Convert the TDOR timestamp from
            ' 2.4 to 2.3, simply by using only
            ' the first 4 bytes, identifying
            ' the year.
            .size = 4
...
Private Function GetNewFrameID( _
...
' The version parameter defines in which ID3v2
' version the tag is written (2.3 or 2.4).
' You possibly will prefer v 2.3 because it has
' greater support by other programs and still
' provides all necessary functionality. For example,
' WinAmp only supports v 2.3.

' The pBuffer parameter can point to a static memory
' buffer which is used if the file has to be rewritten.
' The buffer should be at least as large as the file
' size. If no buffer is provided or if the buffer is
' too small, the memory is allocated dynamic, which
' is slower. Especially when tagging more files,
' it's recommended to allocate a byte array of some
' MB and pass VarPtr(Buffer(0)) for the pBuffer
' parameter and UBound(Buffer) + 1 for the buffer
' size. This is the fastest file rewriting method
' possible.
Public Function WriteID3v2( _
    ByVal FileName As String, _
    ByRef inTag As ID3Tag, _
    ByVal Version As ID3v2_Version, _
    ByVal bClearExistingTag As Boolean, _
    Optional ByVal bIgnoreReadOnly As Boolean = True, _
    Optional ByVal pBuffer As Long = 0, _
    Optional ByVal BufferSize As Long = 0 _
) As Boolean

    Dim i           As Long, fh         As Long
    Dim fp          As Long, fsize      As Long
    Dim TagHeader   As v2TagHeader
    Dim ExistSize   As Long
    Dim arrp        As Long
    Dim WrData()    As Byte
    Dim WrSize      As Long, WrBuf()    As Byte
    Dim BuildTag    As ID3Tag
    Dim tmpHeader   As v2_34FrameHeader
    Dim rdBuf(3)    As Byte
    ...
    ' Never mind on possibly existing appended
    ' tags or footer tags. Just leave them in the
    ' file.
    ExistSize = GetTagSize(fh, 0)
    
    ' Read the existing tag into memory if it
    ...
    If Not ExistSize = 0 And Not bClearExistingTag Then
        ...
        Do
            ...
            If Data2String(VarPtr(rdBuf(0)), 3, ENC_ISO) = "ID3" Then
                ' We've got an ID3 v2 tag.
                ' Write it temporary in the BuildTag
                ' variable.
    ...
    ' dd the size of the existing frames
    ...
    Else
        ...
        ' Add padding, and not too less. A few KB
        ' don't matter in a 3+ MB file, and the
        ' file won't have to be rewritten soon.
    ...
    With BuildTag
        ' Note: we don't use VarPtr(WrData(arrp))
        ' here because if arrp == <array size>,
        ' the function call would fail, although
        ' there is no more data written to the array.
...
' Determines if a frame in memory has to be
' stored in the new written file.
Private Function IsToStore( _
    ByVal fraID As String _
) As Boolean
...
Private Function WriteFrame( _
    ...
    Dim EncFormat   As Byte
    Dim WrOffset    As Long
...
Private Function WriteMemFrame( _
    ByVal pData As Long, _
    ByVal fraIdx As Long, _
    ByVal Version As ID3v2_Version _
) As Long
...
Private Function GetFrameSize( _
    ByVal FrameID As String, _
    ByVal strData As String, _
    ByVal bIncludeHeader As Boolean _
) As Long
...
' See the WriteID3v2 function for a description
' on the Buffer parameters.
Public Function DeleteID3v2( _
    ByVal FileName As String, _
    Optional ByVal bIgnoreReadOnly As Boolean = True, _
    Optional ByVal pBuffer As Long = 0, _
    Optional ByVal BufferSize As Long = 0 _
    ...
    ' Never mind on possibly existing appended
    ' tags or footer tags. Just leave them in
    ' the file.
    ExistSize = GetTagSize(fh, 0)
...
' Synchsafe integers are integers that keep its
' highest bit (bit 7) zeroed, making seven bits
' out of eight available. Thus a 32 bit synchsafe
' integer can store 28 bits of information.
Private Function Data2Long( _
    ByVal pData As Long, _
    ...
    ' Avoid converting wrong synchsafe integers.
    ' If bit 7 of any byte is set, it is not
    ' synchsafe.
    ' However, we can't detect wrong coded values
    ' which have bit 7 zeroed.
    For i = 0 To 3
        If Data(i) And &H80& Then bSynchSafe = False
    Next i

    ' Perform left-shifts, done by multiplication
    ' with the hex values of 2^n. Finally, bit-or
    ' the values.
    If bSynchSafe Then
...
Private Function Long2Data( _
    ByVal SrcValue As Long, _
    ByVal bSynchSafe As Boolean, _
    ByVal pData As Long _
)
    ...
    ' Mask out the corresponding bits and
    ' right-shift them afterwards
    ' (done by division).
    If bSynchSafe Then
...
Private Function Data2String( _
    ByVal pData As Long, _
    ByVal Length As Long, _
    ByVal EncFormat As v2_StrEncoding, _
    Optional ByVal BreakOnNull As Boolean = True _
) As String
    ...
    For i = 0 To Length - 1
        ...
        ' New lines are represented by &0A&
        ' (which is chr(10)) only in ID3 v2 tags.
        ' However, many programs seem to code the
        ' newline with chr(13) & chr(10),
        ' which is the windows default.
        ' Therefore, skip chr(13) and change
        ' chr(10) to vbNewLines.
        ...
        ElseIf EncFormat = ENC_UNICODE_UTF16_BOM Then
            ...
            ' UNICODE text with BOM, double-null
            ' terminated.
...
Private Function String2Data( _
    ByVal SrcStr As String, _
    ByVal MaxLength As Long, _
    ByVal pData As Long, _
    ByVal bTerminate As Boolean _
) As Long
...
Private Function Frame2Genre( _
    ByVal strData As String _
) As String
...
Private Function Genre2Frame( _
    ByVal strGenre As String _
) As String
...
Public Function GetGenreName( _
    ByVal GenreIndex As Long _
) As String
...
Public Function GetGenreIndex( _
...
Public Function ReadMPEGInfo( _
    ByVal FileName As String, _
    ByRef outInfo As MPEGInfo _
) As Boolean

    Dim fh              As Long, fp         As Long
    Dim fsize           As Long
    Dim fraHeader(3)    As Byte
    Dim mVers           As Long, lVers      As Long
    Dim opByte          As Byte, opLong     As Long
    Dim VBRHeaderPos(1) As Long
    Dim VBRHeader(17)   As Byte
    Dim VBRIdent        As String
    Dim SamplesPerFrame As Long
    Dim VBRNrFrames     As Long, VBRBytes   As Long
    Dim RdTag           As v1Tag
    Dim TagHeader       As v2TagHeader
    ...
    Do
        ...
        If fraHeader(0) = &HFF& And (fraHeader(1) And &HE0&) = &HE0& Then
            ...
            ' The position of an possibly existing
            ' VBR header depends on the channel mode.
            ...
            ' Check if there is a VBR header.
            ' If present, use it to read out
    ...
    If fsize >= 128 Then
        ...
        ' The ID3v1 tag is located 128 bytes from
        ' the end of the file, so look at this
        ' position.
        SetFilePointer fh, fsize - 128, 0, FILE_BEGIN
        ReadFile fh, RdTag, Len(RdTag), 0, ByVal 0
        ...
        If Data2String(VarPtr(RdTag.Identifier(0)), 3, ENC_ISO) = "TAG" Then
            ...
            If RdTag.Comment(28) = 0 And Not RdTag.Comment(29) = 0 Then
                ' ID3v1.1 tag: byte 28 of the
                ' comment tag is zeroed, byte
                ' 29 is the track number.
                outInfo.ID3v1Version = 1
            Else
                ' ID3v1.0 tag: contains no track
                ' number.
...
Private Function GetTagSize( _
    ...
    If Data2String(VarPtr(TagHeader.Identifier(0)), 3, ENC_ISO) = "ID3" Then
        ' The size stored in the header excludes
        ' itself, and excludes the footer
        ' (if present).
        ...
        If TagHeader.Version(0) >= 4 Then
            If TagHeader.Flags And &H10& Then
                ' Add the size of the footer
                ' (which is the same size than the
                ' header) to the existing size.
