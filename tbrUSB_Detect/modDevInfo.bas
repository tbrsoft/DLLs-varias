...
Public Type DEVICE_INFORMATION
    ...
    'AMPLIADO
    TotalSizeMB                             As Double
    FreeSpaceMB                             As Double
...
Public Function GetDevInfo(ByVal strDrive As String) As DEVICE_INFORMATION
    ...
    If lngResult Then
        ...
        With GetDevInfo
            ...
            .SerialNumber = udtOut.SerialNumberOffset
            
            Dim TS As Double, TF As Double, VN As String
            RefreshSpacesMB strDrive, TS, TF, VN
            
            .TotalSizeMB = TS
            .FreeSpaceMB = TF
            .VolumeName = VN
            
            'estaría bueno saber más datos
'            udtOut.BusType                 USADO
'            udtOut.CommandQueueing
'            udtOut.DeviceType
'            udtOut.DeviceTypeModifier
'            udtOut.ProductIdOffset         USADO
'            udtOut.ProductRevisionOffset   USADO
'            udtOut.RawDeviceProperties
'            udtOut.RawPropertiesLength
'            udtOut.RemovableMedia          USADO
'            udtOut.SerialNumberOffset
'            udtOut.Size
'            udtOut.VendorIdOffset          USADO
'            udtOut.Version
...
Public Function RefreshSpacesMB(strDrive As String, RetSize As Double, RetFree As Double, RetVolName As String)
    Dim D As Drive
    Set D = FSO.GetDrive(strDrive)
    
    If D.IsReady Then
        RetSize = Round((D.TotalSize / 1048576), 2)
        RetFree = Round((D.FreeSpace / 1048576), 2)
        RetVolName = D.VolumeName
    Else
        RetSize = 0
        RetFree = 0
        RetVolName = "Desconectado"
    End If
End Function
