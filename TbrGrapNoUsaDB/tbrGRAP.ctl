...
Public Sub LoadFromMtx(mtxString() As String, mtxVal() As Single)
    'cargar los datos desde una matriz string, numero
    Dim H As Long
    ...
    For H = 0 To UBound(mtxString)
        ReDim Preserve mCampoString(CB): mCampoString(CB) = mtxString(CB - 1)
        ReDim Preserve mCampoNum(CB): mCampoNum(CB) = mtxVal(CB - 1)
        'ver max min
        If mtxVal(CB - 1) > sMax Then sMax = mtxVal(CB - 1)
        If mtxVal(CB - 1) < sMin Then sMin = mtxVal(CB - 1)
...
Public Sub Mostrar()
    ...
    Dim B As Long
    For B = 1 To CB
        L(B).Caption = mCampoString(B) + " = " + CStr(Round(mCampoNum(B), 2))
