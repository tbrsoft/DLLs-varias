...
{   
	...
	USBINT_UEP = EP_IN|HSHK_EN;	//enable 1 data pipe
	...
	USBINT_BD_IN.ADR = (byte*)&usbint_in;	//set buffer address
	USBINT_BD_IN.Stat._byte = _UCPU|_DAT1;	//set buffer status
...
//same thing, but for the other endpoint.
void USBIntWrite(byte *buffer, byte len)
{
	byte i;	
    //Value of len should be equal to or smaller than USBGEN_EP_SIZE.
    //This check forces the value of len to meet the precondition.
	if(len > USBINT_EP_SIZE)
	    len = USBINT_EP_SIZE;

   //Copy data from user's buffer to dual-ram buffer
    for (i = 0; i < len; i++)
    	usbint_in[i] = buffer[i];

    USBINT_BD_IN.Cnt = len;
    mUSBBufferReady(USBINT_BD_IN);
